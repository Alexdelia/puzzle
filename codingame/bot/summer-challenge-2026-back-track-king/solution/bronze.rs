...
fn main() {
	...
	for _ in 0..height as usize {
		for _ in 0..width as usize {
			...
			let _region_id = parse_input!(inputs[0], i32);
	...
	for i in 0..town_count as usize {
		...
		io::stdin().read_line(&mut input_line).unwrap();
		let inputs = input_line.split(" ").collect::<Vec<_>>();
		let _town_id = parse_input!(inputs[0], i32);
	...
	loop {
		...
		let mut input_line = String::new();
		io::stdin().read_line(&mut input_line).unwrap();
		let _my_score = parse_input!(input_line, i32);
		let mut input_line = String::new();
		io::stdin().read_line(&mut input_line).unwrap();
		let _foe_score = parse_input!(input_line, i32);
		for i in 0..height as usize {
			for j in 0..width as usize {
				...
				let _instability = parse_input!(inputs[1], i32); // region inked (destroyed) when this >= 3.
				let inked = parse_input!(inputs[2], i32); // true if region is destroyed.
				let _part_of_active_connections = inputs[3].trim().to_string(); // if this cell is part of one or more railway connections, this will be town ids (separated by -) in a list separated by commas. e.g. 0-1,1-2,1-3. "x" otherwise.
