...
fn knob<T: std::str::FromStr>(_name: &str, fallback: T) -> T {
	fallback
}
	...
	fn plot(
		...
		while let Some(Reverse((reached, at))) = self.heap.pop() {
			...
			for way in 0..4 {
				...
				let total = reached + stride + ((cell as u32).wrapping_mul(2654435761) >> 30);
...
fn ink_odds(instability: u8) -> f64 {
	match instability {
		0 => 0.10,
		1 => 0.25,
		2 => 0.50,
	...
	fn build(map: &Map, state: &State, tune: &Tune) -> Outlook {
		...
		if tune.scenarios > 0 {
			for region in 0..map.region_cells.len() {
				...
				threats.push((ink_odds(state.instability[region]) * held as f64, region));
			}
		...
		for (rank, (_, region)) in threats.iter().enumerate() {
			let odds = ink_odds(state.instability[*region]);
...
fn pick_disrupt(
	...
	for region in 0..map.region_cells.len() {
		...
		if value > tune.crude_floor && turn <= tune.late_ink {
			...
			if idle.as_ref().is_none_or(|(top, _)| score > *top) {
...
fn decide(
	...
	for _ in 0..3 {
		...
		let Some(choice) = survey(
			map, state, tune, engine, &owner, &outlook, remaining, &clock,
		) else {
			break;
		};
...
fn main() {
	...
	while state.read(&map, &mut input) {
		turn += 1;
		decide(&map, &state, &tune, &mut engine, turn, &mut commands);
