...
struct Ledger {
	name: String,
	rows: Vec<(i32, Cell, [f64; FEATS])>,
	frames: Vec<(Vec<u8>, Vec<u16>)>,
}

impl Ledger {
	fn open() -> Option<Ledger> {
		let dir = std::env::var("BTK_DUMP").ok()?;
		let stamp = std::time::SystemTime::now()
			.duration_since(std::time::UNIX_EPOCH)
			.map(|span| span.as_nanos())
			.unwrap_or(0);
		Some(Ledger {
			name: format!("{}/{}-{}.tsv", dir, std::process::id(), stamp),
			rows: Vec::new(),
			frames: Vec::new(),
		})
	}

	fn frame(&mut self, state: &State) {
		self.frames.push((state.owner.clone(), state.mult.clone()));
	}

	fn claim(&mut self, turn: i32, at: Cell, feat: [f64; FEATS]) {
		self.rows.push((turn, at, feat));
	}
	...
	fn flush(&self) {
		...
		for (turn, at, feat) in &self.rows {
			let mut paid = 0.0;
			for (owner, mult) in self.frames.iter().skip(*turn as usize) {
				if owner[*at as usize] == MINE {
					paid += mult[*at as usize] as f64;
				}
			}
			text.push_str(&format!("{paid}"));
			for worth in feat {
				text.push_str(&format!("\t{worth}"));
			}
			text.push('\n');
		}
		let _ = std::fs::write(&self.name, text);
	}
}
...
fn decide(
	...
	engine.ride = false;
	let (hope, mult) =
		if tune.pre > 0.0 || tune.fill_mult > 0.0 || tune.route_mult > 0 || ledger.is_some() {
	...
	let mut pre = if tune.pre > 0.0 {
		preclaim(map, state, tune, &owner, &hope, &mult, remaining)
	} else {
		None
	};
	...
	if let Some(log) = ledger {
		for &at in &built {
			log.claim(
				turn,
				at,
				features(map, state, &mult, remaining, at as usize),
			);
		}
	}
	for at in &built {
