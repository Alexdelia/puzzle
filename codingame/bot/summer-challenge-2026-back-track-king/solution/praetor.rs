...
impl Tune {
	fn load() -> Tune {
		Tune {
			...
			crude_self: knob("BTK_CRUDE_SELF", 1.0),
			...
			think_ms: knob("BTK_THINK_MS", 28),
			crude_mix: knob("BTK_CRUDE_MIX", 1.0),
			...
			cells: knob("BTK_CELLS", 96),
			cell_hint: knob("BTK_CELL_HINT", 0.5),
			kill_gain: knob("BTK_KILL_GAIN", 1.0),
			kill_bar: knob("BTK_KILL_BAR", 0.0),
			prime_cap: knob("BTK_PRIME_CAP", 4),
			mirror: knob("BTK_MIRROR", 1),
			mirror_ink: knob("BTK_MIRROR_INK", 2),
			cell_inst: knob("BTK_CELL_INST", 3),
			mirror_doom: knob("BTK_MIRROR_DOOM", 2),
			spoil: knob("BTK_SPOIL", 1.0),
			steal: knob("BTK_STEAL", 0.0),
			close: knob("BTK_CLOSE", 0),
			cell_power: knob("BTK_CELL_POWER", 0.0),
			ghost_ms: knob("BTK_GHOST_MS", 0),
			ghost_plots: knob("BTK_GHOST_PLOTS", 0),
			doom_eval: knob("BTK_DOOM_EVAL", 1),
			ghost_plain: knob("BTK_GHOST_PLAIN", 2),
			ink_first: knob("BTK_INK_FIRST", 1),
		}
	}
}
...
fn survey(
	...
	let taken = |engine: &mut Engine, cells: &[Cell], who: u8| {
		engine.trial.copy_from_slice(field);
		for &at in cells {
			engine.trial[at as usize] = who;
		}
		let trial = std::mem::take(&mut engine.trial);
		let worth = appraise(map, engine, &trial, outlook);
		engine.trial = trial;
		(worth - base) / outlook.span
	};
	let weigh = |engine: &mut Engine,
	             cells: &[Cell],
	             paint: u32,
	             loyalty: f64,
	             slack: f64,
	             who: u8,
	             power: f64| {
		let gain = taken(engine, cells, who);
		...
		Some(loyalty * gain * turns / (paint as f64).powf(power))
	};

	let cheap = if tune.cell_power > 0.0 {
		tune.cell_power
	} else {
		tune.paint_power
	};
	...
	for index in 0..map.links.len() {
		...
		for &mode in &live_modes {
			...
			let Some(score) = weigh(engine, &cells, paint, 1.0, slack, MINE, tune.paint_power)
			else {
				continue;
			};
	...
	if tune.cells > 0 && !mult.is_empty() {
		...
		for slot in 0..map.cells {
			...
			if map.is_town[slot]
				...
				|| state.inked[region]
				|| dead[region]
				|| state.instability[region] >= tune.cell_inst
			{
				continue;
			}
			let mut touching = 0;
			for way in 0..4 {
				...
				if next != NO_CELL && (map.is_town[next as usize] || field[next as usize] != EMPTY)
				{
					touching += 1;
				}
			}
			...
			seeds.push((
				tune.cell_hint * mult[slot] as f64 + touching as f64 - map.cost[slot] as f64,
				slot as Cell,
			));
		}
		seeds.sort_by(|a, b| b.0.total_cmp(&a.0).then(a.1.cmp(&b.1)));
		seeds.truncate(tune.cells);
		let mut one = [0 as Cell; 1];
		for &(_, at) in &seeds {
			if clock.elapsed().as_millis() > deadline {
				break;
			}
			one[0] = at;
			let paint = map.cost[at as usize] as u32;
			if let Some(score) = weigh(engine, &one, paint, 1.0, 0.0, MINE, cheap)
	...
	if tune.spoil > 0.0 {
		...
		for &at in ghost {
			...
			if field[at as usize] != FOE {
				continue;
			}
			one[0] = at;
			let paint = map.cost[at as usize] as u32;
			if let Some(score) = weigh(engine, &one, paint, tune.spoil, 0.0, NEUTRAL, cheap)
				&& best.as_ref().is_none_or(|top| score > top.score)
			{
				best = Some(Choice {
					cells: one.to_vec(),
	...
	if tune.steal > 0.0 {
		let mut one = [0 as Cell; 1];
		for &at in ghost_next {
			if clock.elapsed().as_millis() > deadline {
				break;
			}
			if field[at as usize] != EMPTY || state.inked[map.region[at as usize] as usize] {
				continue;
			}
			one[0] = at;
			let paint = map.cost[at as usize] as u32;
			if let Some(score) = weigh(engine, &one, paint, tune.steal, 0.0, MINE, cheap)
				&& best.as_ref().is_none_or(|top| score > top.score)
			{
				best = Some(Choice {
					cells: one.to_vec(),
	...
	for (extra, loyalty) in [(committed, tune.commit_bonus), (contested, tune.preempt)] {
		...
		let paint = extra
			...
			.sum::<u32>();
		if paint > 0
			&& let Some(score) = weigh(engine, extra, paint, loyalty, 0.0, MINE, tune.paint_power)
			&& best.as_ref().is_none_or(|top| score > top.score)
		{
			best = Some(Choice {
				cells: extra.to_vec(),
...
fn foresee(
	...
	let no_mult: Vec<u16> = Vec::new();
	let no_dead: Vec<bool> = vec![false; map.region_cells.len()];
	let found = survey(
		map,
		state,
		tune,
...
fn pick_disrupt(
	...
	let rate = engine.edge(map, owner);
	let outearned = rate < 0;
	let lead = (state.my_score - state.foe_score) as f64;
	let horizon = (MAX_TURNS - turn).max(0) as f64;
	let closing = match tune.close {
		1 => ahead,
		2 => ahead && lead + rate as f64 * horizon <= 0.0,
		_ => ahead && outearned,
	};
	...
	for region in 0..map.region_cells.len() {
		...
		let mut score = value / steps;
		if steps <= 1.0 {
			score *= tune.kill_gain;
			if value < tune.kill_bar {
				continue;
			}
		} else if state.instability[region] + 1 >= tune.prime_cap {
			continue;
		}
...
fn reflect(map: &Map, state: &State) -> State {
	...
	State {
		my_score: state.foe_score,
		foe_score: state.my_score,
		owner,
		mult: state.mult.clone(),
		instability: state.instability.clone(),
		inked: state.inked.clone(),
		foe_seen,
	}
}
...
fn decide(
	...
	let mut foe_ink: Option<u8> = None;
	let mut ghost_cells: Vec<Cell> = Vec::new();
	let mut ghost_next: Vec<Cell> = Vec::new();
	if !shadow && tune.mirror > 0 {
		let theirs = reflect(map, state);
		let mut ghost: Vec<Command> = Vec::new();
		let mut ghost_committed: Vec<Cell> = Vec::new();
		let mut ghost_placed: Vec<Cell> = Vec::new();
		let salt = engine.salt;
		engine.salt = (1 - map.me as u32).wrapping_mul(0x9E37_79B9);
		let mut ghost_tune = *tune;
		ghost_tune.think_ms = if tune.ghost_ms > 0 {
			tune.ghost_ms
		} else {
			tune.think_ms / 3
		};
		ghost_tune.plots = if tune.ghost_plots > 0 {
			tune.ghost_plots
		} else {
			tune.plots / 2
		};
		if tune.ghost_plain > 0 {
			ghost_tune.cells = 0;
			ghost_tune.spoil = 0.0;
			ghost_tune.steal = 0.0;
			ghost_tune.cell_inst = INSTABILITY_THRESHOLD;
			ghost_tune.mirror_doom = 0;
		}
		if tune.ghost_plain > 1 {
			ghost_tune.crude_self = 2.0;
			ghost_tune.crude_mix = 2.0;
			ghost_tune.modes = 255;
		}
		decide(
			map,
			&theirs,
		...
		engine.salt = salt;
		engine.spent = 0;
		for command in &ghost {
			if let Command::Disrupt(region) = command {
				foe_ink = Some(*region);
			}
		}
		for &at in &ghost_placed {
			if owner[at as usize] == EMPTY {
				owner[at as usize] = FOE;
				ghost_cells.push(at);
			}
		}
	}
	let mut dead = vec![false; map.region_cells.len()];
	if let Some(region) = foe_ink
		&& tune.mirror_doom > 0
		&& state.instability[region as usize] + tune.mirror_doom >= INSTABILITY_THRESHOLD
	{
		dead[region as usize] = true;
	}
	...
	let mut my_kill: Option<u8> = None;
	if tune.ink_first > 0 {
		let mut early: Vec<(f64, u8)> = Vec::new();
		let (mine, _) = pick_disrupt(
			map,
			state,
			tune,
			engine,
		...
		if let Some(region) = mine
			&& state.instability[region as usize] + 1 >= INSTABILITY_THRESHOLD
		{
			dead[region as usize] = true;
			my_kill = Some(region);
		}
	...
	for _ in 0..3 {
		...
		let mine_after = if choice.neutral { NEUTRAL } else { MINE };
		...
		for at in cells {
			let price = map.cost[at as usize] as u32;
			if tune.dedup && owner[at as usize] != EMPTY && !choice.neutral {
				continue;
			}
			if price <= budget {
				budget -= price;
				owner[at as usize] = mine_after;
				field[at as usize] = mine_after;
				if !hope.is_empty() {
	...
	while tune.fill && budget > 0 && clock.elapsed().as_millis() <= tune.think_ms {
		let Some(at) = topup(map, state, tune, engine, &owner, &mult, &dead, budget) else {
			break;
		};
	...
	placed.clear();
	placed.extend_from_slice(&built);
	wanted.clear();
	wanted.extend_from_slice(&planned);
	for at in &built {
	...
	if tune.ink_first > 0
		&& !forced
		&& let Some(region) = my_kill
	{
		ink = Some(region);
	}
	if tune.mirror_ink > 0
		&& !forced
...
fn main() {
	...
	let mut wanted: Vec<Cell> = Vec::new();
	let mut turn = 0;
