...
def print_heatmap_distance(map: List[List[int]], s: Tuple[int, int], e: Tuple[int, int]):
...
def print_heatmap_height(map: List[List[int]], s: Tuple[int, int], e: Tuple[int, int], soil: Optional[List[str]] = None):
    for x, r in enumerate(map):
        for y in range(len(r)):
            if (x, y) == s:
                ...
                if soil:
                    print(
                        f"\033[38;2;{intensity};{255 - intensity};0m", end='')
